...
import { VeraCoreEngine } from "../vera/core";
import { RecommendationEngine } from "../vera/recommendations";
import { DashboardDataGenerator } from "../vera/dashboard-data";

export class EsquiloEngine {
  private vera = new VeraCoreEngine();
  private recommendations = new RecommendationEngine();
  private dashboardGenerator = new DashboardDataGenerator();
  ...
  public evaluate(profile: UserFinancialProfile, history: BehavioralHistory): DecisionOutput {
    ...
    // 7. Generate Specific Recommendations (NEW)
    const recommendationSet = this.recommendations.generateRecommendations(profile, {
      debtPressure,
      liquidityAdequacy,
      behavioralConsistency
    });

    const simplifiedScore = this.recommendations.calculateSimplifiedScore(
      debtPressure,
      liquidityAdequacy,
      behavioralConsistency
    );

    const simplifiedBand = this.recommendations.getBand(simplifiedScore);
    ...
    return {
      ...
      // NEW FIELDS (Phase 1)
      simplified_score: simplifiedScore,
      simplified_band: simplifiedBand,
      primary_problem: recommendationSet.problems[0]?.type || this.determineMainProblem(stage, debtPressure, liquidityAdequacy),
      primary_action: recommendationSet.problems[0]?.action || this.determinePrimaryAction(stage, debtPressure, liquidityAdequacy),
      problems: recommendationSet.problems,
      opportunities: recommendationSet.opportunities
    };
  }
  ...
  /**
   * Generate complete dashboard data (Phase 3)
   */
  public getDashboardData(
    ...
    const decision = this.evaluate(profile, history);
    return this.dashboardGenerator.generateDashboardData(userId, profile, decision, undefined, trendData);
  }
}
