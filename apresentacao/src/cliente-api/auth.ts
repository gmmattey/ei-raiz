...
export async function registrar(entrada: RegistrarEntrada): Promise<TokenSaida> {
  const resposta = await apiRequest<TokenSaida>("/api/auth/registrar", {
    method: "POST",
    body: JSON.stringify(entrada),
  });
  saveSession({
    token: resposta.token,
    usuario: { id: "", nome: entrada.nome, email: entrada.email },
  });
...
export function recuperarIniciar(entrada: RecuperarIniciarEntrada): Promise<{ enviado: true }> {
...
export function recuperarConfirmar(entrada: RecuperarConfirmarEntrada): Promise<{ valido: true }> {
...
export function recuperarRedefinir(entrada: RecuperarRedefinirEntrada): Promise<TokenSaida> {
...
export async function solicitarRecuperacaoPorEmail(email: string): Promise<{ enviado: true }> {
  try {
...
export async function solicitarRecuperacaoPorCpf(_cpf: string): Promise<{ enviado: true }> {
  throw new Error("Fluxo de recuperação por CPF indisponível: use o e-mail cadastrado.");
}

export async function redefinirSenha(pin: string, novaSenha: string): Promise<TokenSaida> {
  let email = "";
