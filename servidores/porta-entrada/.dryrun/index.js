...
async function handleAdminRoutes(pathname, request, env, sessao) {
  ...
  const serviceTokenValido = ehRotaCvm && !!env.ADMIN_TOKEN && (request.headers.get("x-admin-token") === env.ADMIN_TOKEN || request.headers.get("authorization") === `Bearer ${env.ADMIN_TOKEN}`);
  ...
  const validarAdmin = /* @__PURE__ */ __name(async () => {
    ...
    if (!sessao) return erro("ACESSO_NEGADO", "Sess\xE3o ausente", 401);
