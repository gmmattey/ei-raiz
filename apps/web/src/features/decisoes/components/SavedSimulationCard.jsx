...
export const SavedSimulationCard = ({ title, type, date, recommendation, onClick, onDelete }) => {
  return (
    <div className="group relative flex flex-col gap-6 rounded-sm border border-[#EFE7DC] bg-white p-6 shadow-sm transition-all hover:border-[#F56A2A] hover:shadow-md md:flex-row md:items-center">
      ...
      <div className="flex items-center gap-4">
        {onDelete && (
          <button 
            ...
            className="flex h-10 w-10 items-center justify-center rounded-sm text-[#0B1218]/20 transition-colors hover:bg-red-50 hover:text-red-500"
          >
            <Trash2 size={18} />
          </button>
        )}
        <button 
          onClick={onClick}
          className="flex items-center gap-2 rounded-sm bg-[#0B1218] px-6 py-3 text-[10px] font-bold uppercase tracking-widest text-white transition-all hover:bg-[#111923]"
        >
