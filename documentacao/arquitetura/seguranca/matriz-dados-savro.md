...
| Backup automático do SO (iCloud/iTunes, iOS) | Sistema operacional | **Mitigado (correção obrigatória do Luiz sobre o achado desta auditoria).** `savro.db` e seus sidecars (`-journal`, `-wal`, `-shm`) são explicitamente excluídos do backup automático via `NSURLIsExcludedFromBackupKey` logo após a abertura do banco (`ExclusaoBackupAutomaticoIOS.kt`). A chave mestra (Keychain, `ThisDeviceOnly`) já estava corretamente excluída antes desta correção. `NSUserDefaults` (preferências não sensíveis — onboarding, política de proteção, timeout) continua no backup padrão do SO, decisão aceita por não conter dado patrimonial nem material criptográfico. | Ver `modelo-ameacas-savro.md` seção 1.1. Distinção explícita: isto é sobre o banco INTERNO — o backup MANUAL do app (`*.savrobackup`, #121) continua funcionando normalmente, por ser ação explícita do usuário. Validação real pendente do job `ios-xcode-macos` da CI (não executável neste ambiente). |
