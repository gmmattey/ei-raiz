...
| `compose-ui-backhandler` | `org.jetbrains.compose.ui:ui-backhandler` | 1.11.1 | `BackHandler` comum (`androidx.compose.ui.backhandler`) — intercepta o botão físico/gesto preditivo de voltar do Android | Apache 2.0 | Android, iOS | JetBrains, ativa | Não | Não | Não | #181 — sem isso, back saía do app direto de qualquer tela em vez de navegar a pilha interna. Não vem transitivo de `compose-foundation`/`compose-ui`, precisou ser declarado à parte. No-op em iOS (sem hardware de voltar) |
